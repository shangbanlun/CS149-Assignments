...
// take in data for (M x N) double data, M for point num, N for dim of a point.
void initData(double *data, int M, int N)
{
	int K = 10;	   // * cluster num
	double *centers = new double[K * N];    // * for store cluster center point data.
	...
	// * randomly generate cluster center point data.
	for (int k = 0; k < K; k++)
	{
		for (int n = 0; n < N; n++)
		{
	...
	for (int m = 0; m < M; m++)
	{
		int startingPoint = rand() % K; // randomly decide point m belong to which cluster.
		for (int n = 0; n < N; n++)	// generate data of point m in dimention n.
		{
...
int main()
{
	...
	const char *pathData = "./data/data.dat";
	readData(pathData, &data, &clusterCentroids, &clusterAssignments, &M, &N,
			 &K, &epsilon);
	...
	logToFile("./bin/start.log", SAMPLE_RATE, data, clusterAssignments,
	...
	logToFile("./bin/end.log", SAMPLE_RATE, data, clusterAssignments,
			  clusterCentroids, M, N, K);
