...
typedef struct
{
	...
	double *data;	// * (M x N)
	double *clusterCentroids;	// * (K x N)
	int *clusterAssignments;	// * (M)
	double *currCost;    // * 
	int M, N, K;
} WorkerArgs;
...
{
	double *minDist = new double[args->M];	// * minDist for points.
...
{
	int *counts = new int[args->K];	   // * (K) how many points assigned to each cluster.
	...
	for (int m = 0; m < args->M; m++)
	{
		int k = args->clusterAssignments[m];	// * get the cluster which point m belong to.
		for (int n = 0; n < args->N; n++)
		{
				  ...
				  int M, int N, int K, double epsilon)
{
	...
	// Used to track convergence, cost is the sum of dist between point and corresponding center for a cluster.
	...
	int iter = 0;
	while (!stoppingConditionMet(prevCost, currCost, epsilon, K))	// * if cost of every cluster converge
	{
