...
void clampedExpVector(float *values, int *exponents, float *output, int N)
{
	...
	__cs149_vec_float x;
	__cs149_vec_int y;
	__cs149_vec_float vecResult = _cs149_vset_float(1.0f);
	...
	__cs149_mask isExpZeroMask, nonExpZeroMask, maskGtMax;
	...
	for (int i = 0; i < N; i += VECTOR_WIDTH)
	{
		...
		_cs149_veq_int(isExpZeroMask, y, vecIntZero, maskAll);
		isExpZeroNum = _cs149_cntbits(isExpZeroMask);
		
		while (isExpZeroNum != VECTOR_WIDTH) {
			nonExpZeroMask = _cs149_mask_not(isExpZeroMask);
			_cs149_vmult_float(vecResult, vecResult, x, nonExpZeroMask);
			_cs149_vsub_int(y, y, vecIntOne, nonExpZeroMask);

			_cs149_veq_int(isExpZeroMask, y, vecIntZero, maskAll);
			isExpZeroNum = _cs149_cntbits(isExpZeroMask);
		}
		
		_cs149_vgt_float(maskGtMax, vecResult, vecFloatMax, maskAll);
		_cs149_vmove_float(vecResult, vecFloatMax, maskGtMax);
		_cs149_vstore_float(output + i, vecResult, maskAll);
		vecResult = _cs149_vset_float(1.0f);
	}
}
...
float arraySumVector(float *values, int N)
{
	...
	__cs149_vec_float x;
	__cs149_vec_float haddResult;
	__cs149_vec_float interleaveResult;
	float temp[VECTOR_WIDTH];
	int iterNum = log(VECTOR_WIDTH) / log(2);
	int iterIndex;
	...
	for (int i = 0; i < N; i += VECTOR_WIDTH)
	{
		iterIndex = iterNum;
		_cs149_vload_float(x, values + i, maskAll);
		_cs149_hadd_float(haddResult, x);
		while (iterIndex > 1) {
			_cs149_interleave_float(interleaveResult, haddResult);
			_cs149_hadd_float(haddResult, interleaveResult);
			iterIndex -= 1;
		}
		
		_cs149_vstore_float(temp, haddResult, maskFirst);
		sum += *temp;
	}
